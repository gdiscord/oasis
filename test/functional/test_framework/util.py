...
def assert_fee_amount(fee, tx_size, fee_per_kB):
    ...
    if fee < target_fee:
        raise AssertionError("Fee of %s XOS too low! (Should be %s XOS)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    if fee > (tx_size + 20) * fee_per_kB / 1000:
        raise AssertionError("Fee of %s XOS too high! (Should be %s XOS)" % (str(fee), str(target_fee)))
...
def append_config(dirname, n, options):
    datadir = get_datadir_path(dirname, n)
    with open(os.path.join(datadir, "oasis.conf"), 'a', encoding='utf8') as f:
