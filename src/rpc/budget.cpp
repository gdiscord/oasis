...
UniValue preparebudget(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 6)
        throw std::runtime_error(
            ...
            "5. \"address\":     (string, required) OASIS address to send payments to\n"
    ...
    if (!pwallet->CreateBudgetFeeTX(wtx, nHash, keyChange, false)) { // 50 XOS collateral for proposal
...
UniValue getbudgetprojection(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "    \"TotalPayment\": xxx.xxx,      (numeric) Total payment amount in XOS\n"
            "    \"MonthlyPayment\": xxx.xxx,    (numeric) Monthly payment amount in XOS\n"
            ...
            "    \"Allotted\": xxx.xxx,           (numeric) Amount of XOS allotted in current period\n"
            "    \"TotalBudgetAllotted\": xxx.xxx (numeric) Total XOS allotted\n"
            "  }\n"
            "  ,...\n"
            "]\n"
