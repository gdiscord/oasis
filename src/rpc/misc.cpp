...
UniValue validateaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"isstaking\" : true|false,       (boolean) If the address is a staking address for OASIS cold staking -only if is standard address-\n"
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "oasis-cli and the GUI. There is no server-side difference."
        );
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ --------
    ...
    { "util",               "validateaddress",        &validateaddress,        true,  {"oasisaddress"} }, /* uses wallet if enabled */
    { "util",               "verifymessage",          &verifymessage,          true,  {"oasisaddress","signature","message"} },
