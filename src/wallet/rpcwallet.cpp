...
UniValue getaddressinfo(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
                ...
                "{Result:\n"
                "  \"address\" : \"address\",              (string) The OASIS address validated.\n"
...
UniValue setlabel(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"       (string, required) The oasis address to be associated with a label.\n"
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"     (string, required) The oasis address to send to.\n"
            ...
            "                             The recipient will receive less XOSs than you enter in the amount field.\n"
...
UniValue delegatestake(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 7)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"staking_addr\"      (string, required) The oasis staking address to delegate.\n"
            "2. \"amount\"            (numeric, required) The amount in XOS to delegate for staking. eg 100\n"
            "3. \"owner_addr\"        (string, optional) The oasis address corresponding to the key that will be able to spend the stake.\n"
...
UniValue shieldsendmany(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
                ...
                "      \"amount\":amount    (numeric, required) The numeric amount in " + "XOS" + " is the value\n"
                ...
                "                           Those recipients will receive less XOS than you enter in their corresponding amount field.\n"
...
UniValue signmessage(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"     (string, required) The oasis address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"    (string, required) The oasis address for transactions.\n"
...
UniValue getreceivedbylabel(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in XOS received for this label.\n"
...
UniValue getbalance(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || (request.params.size() > 4 ))
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in XOS received for this wallet.\n"
...
UniValue getcoldstakingbalance(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || (request.params.size() != 0))
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in XOS received for this wallet in P2CS contracts.\n"
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 6)
        throw std::runtime_error(
            ...
            "\nSend to multiple destinations. Recipients are transparent or shield OASIS addresses.\n"
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The oasis address (either transparent or shield) is the key,\n"
            "                                     the numeric amount in XOS is the value\n"
            "      ,...\n"
            "    }\n"
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "2. \"keys\"         (string, required) A json array of oasis addresses or hex-encoded public keys\n"
            "     [\n"
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 6) throw std::runtime_error(
            ...
            "[\n"
            "  {\n"
            "    \"address\":\"oasisaddress\",    (string) The oasis address of the transaction.\n"
            "    \"category\":\"category\",      (string) The transaction category (send|receive|orphan|immature|generate).\n"
            "    \"amount\": x.xxx,          (numeric) The amount in XOS. This is negative for the 'send' category, and positive\n"
...
UniValue fundrawtransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "   {\n"
            "     \"changeAddress\"     (string, optional, default pool address) The OASIS address to receive the change\n"
            "     \"changePosition\"    (numeric, optional, default random) The index of the change output\n"
            "     \"includeWatching\"   (boolean, optional, default false) Also select inputs which are watch only\n"
            "     \"lockUnspents\"      (boolean, optional, default false) Lock selected unspent outputs\n"
            "     \"feeRate\"           (numeric, optional, default 0=estimate) Set a specific feerate (XOS per KB)\n"
    ...
    if (request.params.size() > 1) {
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!IsValidDestination(changeAddress))
                throw JSONRPCError(RPC_INVALID_PARAMETER, "changeAddress must be a valid OASIS address");
        }
...
UniValue getwalletinfo(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"autocombine_threshold\": x.xxx,          (numeric) the current autocombine threshold in XOS\n"
...
UniValue autocombinerewards(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("autocombinerewards")) {
        if (request.fHelp) {
            throw std::runtime_error("autocombinerewards (Deprecated, will be removed in UPGRADE_V4_0. To use this command, start oasisd with -deprecatedrpc=autocombinerewards)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "autocombinerewards is deprecated and will be removed in UPGRADE_V4_0. To use this command, start oasisd with -deprecatedrpc=autocombinerewards");
    }
...
UniValue setautocombinethreshold(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.empty() || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "  \"threshold\": n.nnn,         (numeric) auto-combine threshold in XOS\n"
...
UniValue getautocombinethreshold(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || !request.params.empty())
        throw std::runtime_error(
            ...
            "{\n"
            "  \"enabled\": true|false,        (boolean) true if auto-combine is enabled, otherwise false\n"
            "  \"threshold\": n.nnn            (numeric) the auto-combine threshold amount in XOS\n"
            "}\n"
