...
// Copyright (c) 2019-2021 The OASIS developers
...
bool BerkeleyEnvironment::Open(bool retry)
{
    ...
    if (!LockDirectory(pathIn, ".walletlock")) {
        LogPrintf("Cannot obtain a lock on wallet directory %s. Another instance of OASIS may be using it.\n", strPath);
        return false;
    }
...
BerkeleyBatch::BerkeleyBatch(BerkeleyDatabase& database, const char* pszMode, bool fFlushOnCloseIn) : pdb(nullptr), activeTxn(nullptr)
{
...

    {
        ...
        if (pdb == nullptr) {
            ...
            // OASIS from opening the same data file through another
