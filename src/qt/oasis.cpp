...
#ifdef ENABLE_WALLET
#include "qt/oasis/governancemodel.h"
...
{
    window = new OASISGUI(networkStyle, nullptr);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &OASISGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &OASISGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &OASISGUI::handlePaymentRequest);
        connect(window, &OASISGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(nullptr, "Runaway exception", QObject::tr("A fatal error occurred. OASIS can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    ...
    /// 6. Determine availability of data and blocks directory and parse oasis.conf
