...
QString formatBalance(CAmount amount, int nDisplayUnit, bool isZxos)
{
    return (amount == 0) ? ("0.00 " + BitcoinUnits::name(nDisplayUnit, isZxos)) : BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, amount, false, BitcoinUnits::separatorAlways, true, isZxos);
}

QString formatBalanceWithoutHtml(CAmount amount, int nDisplayUnit, bool isZxos)
{
    return (amount == 0) ? ("0.00 " + BitcoinUnits::name(nDisplayUnit, isZxos)) : BitcoinUnits::floorWithUnit(nDisplayUnit, amount, false, BitcoinUnits::separatorAlways, true, isZxos);
}
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter OASIS address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
bool openConfigfile()
{
    return openFile(GetConfigFile(gArgs.GetArg("-conf", OASIS_CONF_FILENAME)), true);
}
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::TESTNET)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "OASIS (testnet).lnk";
    else if (chain == CBaseChainParams::REGTEST)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "OASIS (regtest).lnk";
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart) {
        ...
        if (gArgs.GetBoolArg("-testnet", false))
            optionFile << "Name=OASIS (testnet)\n";
        else if (gArgs.GetBoolArg("-regtest", false))
            optionFile << "Name=OASIS (regtest)\n";
        else
