...
SettingsFaqWidget::SettingsFaqWidget(OASISGUI* parent, ClientModel* _model) :
    QDialog(parent),
    ...
    clientModel(_model)
{
    ...
    setCssProperty({
              ...
              ui->labelSubtitle_UnspendableXOS,
    ...
    setCssProperty({
              ...
              ui->labelContent_UnspendableXOS,
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("OASIS is a form of digital online money using blockchain technology "
               "that can be easily transferred globally, instantly, and with near "
               "zero fees. OASIS incorporates market leading security & "
               "privacy and is also among the PoS (Proof of Stake) Cryptocurrencies "
               "to implement Sapling(SHIELD), a zk-SNARKs based privacy protocol.")) +
        formatFAQParagraph(
            tr("OASIS utilizes a Proof of Stake (PoS) consensus system algorithm, "
    ...
    QString unspendableXOSContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received OASIS requires 6 confirmations on the network "
               "to become eligible for spending which can take ~6 minutes.")) +
        formatFAQParagraph(
            tr("Your OASIS wallet also needs to be completely synchronized "
               "to see and spend balances on the network.")));
    ui->labelContent_UnspendableXOS->setText(unspendableXOSContent);
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of OASIS with a minimum of 480 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("oasis-cli", "<span style=\"font-style:italic\">getstakingstatus</span>")));
    ...
    QString supportContent = formatFAQContent(
        formatFAQParagraph(
            tr("We have support channels in most of our official chat groups, for example %1")
                .arg("<a style='color: #b088ff' href='https://discord.gg/5SgtbHK'>" + tr("#support in our Discord") + "</a>.")));
    ...
    QString masternodeContent = formatFAQContent(
        formatFAQParagraph(
            tr("A masternode is a computer running a full node %1 wallet with a "
               ...
               "regularly. These services include:")
                .arg(PACKAGE_NAME)
                .arg(GUIUtil::formatBalance(clientModel->getMNCollateralRequiredAmount(), BitcoinUnits::XOS)) +
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in OASIS Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the OASIS network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
    ...
    connect(ui->pushButton_UnspendableXOS, &QPushButton::clicked, [this](){onFaqClicked(ui->widget_UnspendableXOS);});
    ...
    if (parent)
        connect(parent, &OASISGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
