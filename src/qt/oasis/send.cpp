    ...
    coinIcon(new QPushButton())
{
    ...
    ui->btnUri->setSubTitleClassAndText("text-subtitle", tr("Parse a OASIS URI"));
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onXOSSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onXOSSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent XOSs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res) {
                inform(tr("Error generating address to shield XOSs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent XOSs to shield."));
    }
}
...
void SendWidget::onXOSSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto xosAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(xosAdd) || isStakingAddr) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(xosAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
