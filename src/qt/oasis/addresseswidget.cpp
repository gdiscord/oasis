...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto xosAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(xosAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(xosAdd);
        if (walletModel->updateAddressBookLabels(xosAdd, label.toUtf8().constData(),
