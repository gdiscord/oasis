    ...
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquarePiv, "square-chart-xos");
    setCssProperty(ui->labelSquareMN, "square-chart-mn");
    setCssProperty(ui->labelPiv, "text-chart-xos");
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair XOS, MN Reward
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            xos = (pair.first != 0) ? pair.first / 100000000 : 0;
        ...
        chartData->valuesPiv.append(xos);
        chartData->valuesMN.append(mn);

        int max = std::max(xos, mn);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    forceUpdateStyle({ui->labelAmountXOS, ui->labelAmountMN});
    ui->labelAmountXOS->setText(GUIUtil::formatBalance(chartData->totalPiv, nDisplayUnit));
