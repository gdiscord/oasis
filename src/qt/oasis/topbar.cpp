...
#include "qt/oasis/balancebubble.h"
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopXOS, ui->labelAmountTopShieldedXOS}, "amount-small-topbar");
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    CAmount xosAvailableBalance = newBalance.balance - nLockedBalance;
    QString totalXOS = GUIUtil::formatBalance(xosAvailableBalance, nDisplayUnit);
    QString totalTransparent = GUIUtil::formatBalance(xosAvailableBalance - newBalance.shielded_balance);
    ...
    ui->labelAmountTopXOS->setText(totalTransparent);
    ui->labelAmountTopShieldedXOS->setText(totalShielded);
    
    // Expanded
    /* OASIS merged "Pending" and "Immature" into a single GUI balance, to simplify the experience for the user */
    // Locked
    ui->labelLockedXOS->setText(GUIUtil::formatBalance(nLockedBalance, nDisplayUnit));
    // Pending + Immature
    ui->labelPendingXOS->setText(GUIUtil::formatBalance((newBalance.unconfirmed_balance + newBalance.immature_balance), nDisplayUnit));
}
