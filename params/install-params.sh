...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/OASISParams"
    else
        PARAMS_DIR="$HOME/.oasis-params"
    fi
fi
...
function main() {
    ...
    cat <<EOF
OASIS - install-params.sh

This script will install the OASIS zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common OASIS zkSNARK parameters. Note that it is
